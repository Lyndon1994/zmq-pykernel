...
class Console(code.InteractiveConsole):
    ...
    def recv_output(self):
        while True:
            ...
            # print("recv_output", omsg, file=sys.stderr)
            if omsg is None:
                break
            self.handle_output(omsg)

    def handle_reply(self, rep):
        # print("handle_reply:", rep)
    ...
    def runcode(self, code):
        ...
        omsg = self.session.send(self.request_socket, "execute_request", dict(code=src))
...
class InteractiveClient(object):
    def __init__(self, session, request_socket, sub_socket):
        ...
        self.console = Console(
            None, "<zmq-console>", session, request_socket, sub_socket
        )
...
def main():
    ...
    sub_socket.setsockopt(zmq.SUBSCRIBE, "".encode('ascii'))
